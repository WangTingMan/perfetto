...
class FilemapMapPagesFtraceEvent_Decoder : public ::protozero::TypedProtoDecoder</*MAX_FIELD_ID=*/5, /*HAS_NONPACKED_REPEATED_FIELDS=*/false> {
 public:
  FilemapMapPagesFtraceEvent_Decoder(const uint8_t* data, size_t len) : TypedProtoDecoder(data, len) {}
  explicit FilemapMapPagesFtraceEvent_Decoder(const std::string& raw) : TypedProtoDecoder(reinterpret_cast<const uint8_t*>(raw.data()), raw.size()) {}
  explicit FilemapMapPagesFtraceEvent_Decoder(const ::protozero::ConstBytes& raw) : TypedProtoDecoder(raw.data, raw.size) {}
  bool has_pathbuf() const { return at<1>().valid(); }
  ::protozero::ConstChars pathbuf() const { return at<1>().as_string(); }
  bool has_mtime() const { return at<2>().valid(); }
  uint64_t mtime() const { return at<2>().as_uint64(); }
  bool has_i_ino() const { return at<3>().valid(); }
  uint64_t i_ino() const { return at<3>().as_uint64(); }
  bool has_i_generation() const { return at<4>().valid(); }
  uint32_t i_generation() const { return at<4>().as_uint32(); }
  bool has_index() const { return at<5>().valid(); }
  uint64_t index() const { return at<5>().as_uint64(); }
};

class FilemapMapPagesFtraceEvent : public ::protozero::Message {
 public:
  using Decoder = FilemapMapPagesFtraceEvent_Decoder;
  enum : int32_t {
    kPathbufFieldNumber = 1,
    kMtimeFieldNumber = 2,
    kIInoFieldNumber = 3,
    kIGenerationFieldNumber = 4,
    kIndexFieldNumber = 5,
  };
  static constexpr const char* GetName() { return ".perfetto.protos.FilemapMapPagesFtraceEvent"; }
